...
subfol='final';
outdir='/imaging/rowe/users/ek01/prv/ntad/hip_subfields/';
...
cog_fname='/imaging/rowe/users/ek01/prv/ntad/hip_subfields/cog_data.mat'; % remaining subject data, variables
...
%     r_hem_res(:,k)=mat2gray(mdl.Residuals.Raw); k=k+1;
    ...
    mdl = fitlm([l_hem_data.ICV], l_hem_data{:,f});
    l_hem_res(:,k)=mat2gray(mdl.Residuals.Raw); 
    
    mdl = fitlm([r_hem_data.ICV], r_hem_data{:,f});
    r_hem_res(:,k)=mat2gray(mdl.Residuals.Raw); k=k+1;
end
...
for f=1:size(l_hem_res,2)
    ...
    print(gcf,[outdir 'LH_' fields{f} '_ICVcor_boxplots.bmp'],'-dbmp','-r300'); close(gcf)
    ...
    set(gca, 'XtickLabel', {'C','F','MCI','AD'})
    title(fields{f}); ylim([0 1]); 
    print(gcf,[outdir 'RH_' fields{f} '_ICVcor_boxplots.bmp'],'-dbmp','-r300');close(gcf)
